...
> 本ドキュメントは [`01_requirements.md`](01_requirements.md)（要件定義書）を補完し、実装済みの各モジュールの内部構造・動作仕様・画面仕様・API仕様を網羅的に記載した詳細設計書です。
...
- **frontend/** — React 19 + Vite + Tailwind CSS SPA (`http://localhost:5173`)
- **backend/** — FastAPI + SQLAlchemy 2.0 + SQLite (`http://localhost:8000`)
- フロントエンドはaxiosで `X-User-Id` ヘッダー付きでREST APIを呼び出す
...
| テストユーザー | 管理者 admin@example.com, 一般 member@example.com (PW: password123) |
| テストプロジェクト | Webシステム刷新プロジェクト |
| メンバー割当 | admin=管理者, member=一般 |
...
**ドラッグ完了時**: `onTaskChange(taskId, { planned_start, planned_end, progress })` を呼び出し、PATCH APIでサーバーへ永続化。

#### TaskFormModal (`TaskFormModal.jsx`)
...
#### TaskDetailPanel (`TaskDetailPanel.jsx`)
...
│  (開閉トグル付)    │                                                  │
│                  │  ┌─ ヘッダー ──────────────────────────────────┐  │
│  ユーザー切替      │  │ プロジェクト名  すべて折りたたむ 全展開  +タスク追加│  │
│                  │  └──────────────────────────────────────────┘  │
│  プロジェクト一覧   │                                                  │
│                  │  ┌─ 左ペイン ─── ┤ ├─── 右ペイン ────────────┐   │
...
| Method | Path | 説明 |
|:------|:-----|:----|
| POST | `/api/v1/auth/login` | Email/PW検証、成功時にユーザー情報を返す |
| POST | `/api/v1/users` | 新規ユーザー作成（パスワードはハッシュ化して保存） |
| GET | `/api/v1/users` | 全ユーザー一覧取得 |
| GET | `/api/v1/statuses` | ステータスマスタ一覧取得 (sort_order順) |

### 6.2 プロジェクト

| Method | Path | 説明 |
|:------|:-----|:----|
| GET | `/api/v1/projects` | 自身が参加するプロジェクト一覧 |
| POST | `/api/v1/projects` | プロジェクト新規作成（作成者はAdmin権限自動付与） |
| GET | `/api/v1/projects/{pid}/members` | プロジェクトメンバー一覧取得 |
| POST | `/api/v1/projects/{pid}/members` | 新規メンバー追加 |
| GET | `/api/v1/projects/{pid}/export` | タスクツリー一括JSON出力 |
| POST | `/api/v1/projects/{pid}/import` | JSONタスクツリーを読み込み一括登録 |

### 6.3 WBS・タスク

| Method | Path | 説明 |
|:------|:-----|:----|
| GET | `/api/v1/projects/{pid}/wbs` | タスクツリー取得（クエリ: `parent_id`, `all_tasks`） |
| POST | `/api/v1/projects/{pid}/tasks` | 新規タスク作成（L1/L2は管理者のみ） |
| PATCH | `/api/v1/tasks/{task_id}` | タスク部分更新（L1/L2は管理者のみ） |
| DELETE | `/api/v1/tasks/{task_id}` | タスク削除（カスケード削除） |
| POST | `/api/v1/tasks/{task_id}/deliverables` | 成果物リンク登録 |
| DELETE | `/api/v1/deliverables/{deliverable_id}` | 成果物リンク削除 |
...
[ユーザーが「全展開」ボタンをクリック]
  └─ GET /api/v1/projects/{pid}/wbs?all_tasks=true
      ...
      └─ rawTasks + expandedIds を同時更新 → 全タスクが展開状態で表示
```
